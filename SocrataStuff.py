    ...
    def connectToSocrata(self):
        ...
        with open(clientConfigFile,  'r') as stream:
            try:
                ...
                if(client_items['timeout']):
                    timeout = client_items['timeout']
                else:
                    timeout =  10
                client = Socrata(client_items['url'],  client_items['app_token'], username=client_items['username'], password=base64.b64decode(client_items['password']), timeout=timeout)
                return client
